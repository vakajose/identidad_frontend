...
@Component({
  ...
  template: `
    <ng-container>
      <div *ngIf="root && item.visible !== false" class="layout-menuitem-root-text">{{item.label}}
        <i *ngIf="(!appMain.sidebarActive && app.layoutMode !== 'static' && !appMain.isMobile())" class="pi pi-ellipsis-h"></i>
      </div>
      ...
      <ul *ngIf="(item.items || active) && item.visible !== false"
          [@children]="(appMain.isHorizontal() && !appMain.isMobile() && root) ? (active ? 'visible' : 'hidden') : (root ? 'visible' :active ? 'visibleAnimated' : 'hiddenAnimated')">
...
export class AppMenuitemComponent implements OnInit, OnDestroy{
  ...
  constructor(public app: AppComponent, public appMain: AppMainComponent, public router: Router, private menuService: AppMenuService) {
    this.menuSourceSubscription = this.menuService.menuSource$.subscribe(key => {
      // deactivate current active menu
      if (this.active && this.key !== key && key.indexOf(<string>this.key) !== 0) {
        this.active = false;
      }
    });
  ...
  itemClick(event: Event) {
    ...
    // notify other items
    this.menuService.onMenuStateChange(<string>this.key);
    ...
    if (this.item.items) {
      ...
      if (this.appMain.isMobile()) {
        ...
        this.appMain.mobileMenuActive = false;
      }
